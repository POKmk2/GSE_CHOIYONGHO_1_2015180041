...
void SceneMgr::collisionCheck()
{
	for (int i = 0; i < CharacterArraySize; ++i)
	{
		...
		{
			if (CharacterArray[i]->collisionCheck(BuildingArray[j]))//ƒ≥∏Ø≈Õ-∫Ùµ˘ √Êµπ√º≈©
			{
		...
		{
			if (CharacterArray[i]->collisionCheck(BulletArray[j]))//ƒ≥∏Ø≈Õ-∫“∑ø √Êµπ√º≈©
			{
	...
	for (int i = 0; i < 2; ++i)
	{
		for (int j = 0; j < ArrowArraySize[i]; ++j)
		{
			for (int k = 0; k < CharacterArraySize; ++k)//ƒ≥∏Ø≈Õ-»≠ªÏ √Êµπ√º≈©
			{
				if (CharacterArray[k]->collisionCheck(ArrowArray[i][j])) //¿⁄Ω≈¿Ã Ω »≠ªÏ »Æ¿Œ
				{
			...
			{
				if (ArrowArray[i][j]->collisionCheck(BuildingArray[k]))//∫Ùµ˘-»≠ªÏ √Êµπ√º≈©
				{
